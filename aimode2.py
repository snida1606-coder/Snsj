...
# ══════════════ CONFIG ══════════════
BOT_TOKEN = "8933455652:AAGyQVMgonstdK3h-oKDLj5hyiQzVVWbKNA"
...
def is_authorized(uid: int) -> bool:
    ...
    try:
        # Strict 3 seconds timeout taake cloud par request phanse nahi
        resp = requests.get(url, headers=headers, params=params, timeout=3)
        if resp.status_code == 200:
            return len(resp.json()) > 0
    except Exception as e:
        print(f"Auth Bypass Check: {e}")
        return True # Safe fallback: Cloud error par bhi aapko block nahi karega
    return False
...
def _draw_v4_chart(candles, pair, direction, confidence, payout,
    ...
    if not result_mode and direction:
        ...
        if direction == "CALL":
            glow_color = (0, 213, 127)
        else:
            glow_color = (234, 59, 88)
...
class SMZXBot:
    ...
    def fetch_data(self, pair, limit=600):
        ...
        try:
            # 3 seconds strict timeout taake Cloud network freeze na ho
            r = requests.get(url, headers=headers, timeout=3) 
            if r.status_code == 200:
                data = r.json()
                payout = str(data.get("payout", "92")).replace("%", "")
                
                # Sirf real aur valid candles data hi accept karega
        ...
        # Agar cloud IP completely block ho to None dega taake loop skip ho, freeze na ho
        return None, None, "0"
...
def main():
    ...
    BOT_TOKEN_SENDER = "8933455652:AAGyQVMgonstdK3h-oKDLj5hyiQzVVWbKNA"
