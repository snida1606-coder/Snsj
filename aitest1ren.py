...
class UserTelegramSender:
    ...
    def resolve_entity(self, identifier):
        """Resolve (and cache) an entity on THIS client's own session.
        The bot's Bot-API client and this Telethon user-account client have
        completely separate entity caches, so a channel resolved via
        context.bot.get_chat() is NOT automatically resolvable here. This
        method fixes that by resolving via Telethon directly, refreshing
        dialogs once if the entity isn't cached yet (covers channels/groups
        this account is already a member of but hasn't "seen" this run)."""
        if not self.ready:
            return None
        async def _resolve():
            try:
    ...
    def start_with_bot_token(self, api_id, api_hash, bot_token):
        ...
        async def init():
            try:
                ...
                session_string = os.environ.get("TELETHON_BOT_SESSION", "").strip()
                if session_string:
                    # Reuse a saved session -> NO ImportBotAuthorizationRequest
                    # on this login, so restarts never burn flood-wait budget.
                    self.client = TelegramClient(StringSession(session_string), api_id, api_hash)
                else:
                    # First-ever run: no saved session yet, fall back to file
                    # session so at least local/manual runs still work.
                ...
                if not session_string:
                    # Print the exportable session string ONCE so it can be
                    # copied into your hosting platform's env var
                    # (TELETHON_BOT_SESSION) — works the same on Render,
                    # Railway, or anywhere else. After that, every future
                    # boot reuses it -> no more fresh bot-auth calls -> no
                    # more FloodWaitError.
                    try:
                        exported = StringSession.save(self.client.session)
                        print(f"{Fore.YELLOW}{'=' * 80}\n"
                              f"[ACTION NEEDED] Copy this into your platform's env var TELETHON_BOT_SESSION:\n"
                              f"{exported}\n"
                              f"{'=' * 80}{Style.RESET_ALL}")
                    except Exception as _export_err:
                        print(f"[!] Could not export session string: {_export_err}")

                while True:
                    await asyncio.sleep(60)
            except FloodWaitError as _flood_err:
                # Do NOT crash the whole process for this. The main bot
                # (python-telegram-bot polling) works independently of
                # Telethon, so we just disable the Telethon-only "fancy
                # formatting" sender for this run and keep everything
                # else alive. self.ready stays False; TelegramSender's
                # send_message/send_file/etc already no-op safely when
                # not ready (see _run_async).
                self._start_error = _flood_err
                print(f"{Fore.RED}[!] Telethon FloodWaitError: must wait "
                      f"{_flood_err.seconds}s before next bot-auth attempt. "
                      f"Skipping Telethon sender for this run — main bot "
                      f"still works normally.{Style.RESET_ALL}")
                # swallow it: do not re-raise
            except Exception as _init_err:
                self._start_error = _init_err
                print(f"{Fore.RED}[!] Telethon sender failed to start: "
                      f"{_init_err}. Main bot continues without it.{Style.RESET_ALL}")
                # swallow it here too: a broken fancy-sender must never
                # take down the whole bot process.
        ...
        def run_loop():
            ...
            try:
                ...
                self._start_error = self._start_error or _loop_err
        t = threading.Thread(target=run_loop, daemon=True)
        t.start()
        timeout = 30
        start_time = time.time()
        while not self.ready and self._start_error is None and time.time() - start_time < timeout:
            time.sleep(0.5)
        # NOTE: unlike before, we never raise here. If Telethon couldn't
        # start (flood wait, bad token, timeout, etc.) we just log it and
        # let main() carry on — the PTB bot (run_polling) is the real bot
        # and does not depend on this sender being ready.
        if self._start_error is not None:
            print(f"{Fore.YELLOW}[i] Continuing without Telethon sender "
                  f"({self._start_error}).{Style.RESET_ALL}")
        elif not self.ready:
            print(f"{Fore.YELLOW}[i] Telethon sender init timed out after "
                  f"{timeout}s — continuing without it.{Style.RESET_ALL}")
...
async def global_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---- Auto Trade per-strategy parameter states (mirror main-menu params) ----
        ...
        # The bot's Bot-API client and the Telethon user-account client
        # (user_sender, used to actually SEND auto-signal messages) have
        # separate entity caches. Resolving the channel above only cached
        # it for the bot — resolve it on user_sender's own session too,
        # otherwise auto-signal sends later fail with:
        #   "Could not find the input entity for PeerChannel(channel_id=...)"
        telethon_entity = user_sender.resolve_entity(channel_input)
        if telethon_entity is None:
            telethon_entity = user_sender.resolve_entity(chat_id)
        if telethon_entity is None:
            warn = (
                "⚠️ Bot se channel connect ho gaya hai, lekin premium "
                "account (jiska session signals bhejne ke liye use hota "
                "hai) is channel/group ka member nahi hai ya use access "
                "nahi hai.\n\n"
                "Please us account ko is channel/group mein add/join karo, "
                "phir channel dobara set karo (/start se)."
            )
            await update.message.reply_text(warn)
            return

        # Store the already-resolved Telethon entity, not the bare bot-API
        # chat_id — this avoids re-resolving (and re-failing) on every send.
        context.user_data['auto_target_id'] = telethon_entity
...
def main():
    ...
    try:
        ...
        # Belt-and-suspenders: even if something above ever raises again,
        # never let the fancy-formatting sender take the whole bot down.
        print(f"{Fore.RED}[!] Telethon sender start raised: {_sender_err}. "
    ...
    def _unauthorized_userdata_janitor():
        """
        python-telegram-bot keeps a small dict in memory PERMANENTLY for
        every user_id that has ever interacted (via context.user_data) —
        this never gets freed on its own. With 5k+ outsiders repeatedly
        hitting /start or buttons, that's 5k+ entries sitting in RAM
        forever, on top of whatever the real 30 authorized users use.

        This runs in its own thread, once every 30 minutes, and only
        removes entries for users who are NOT authorized. It never touches
        any handler, never runs on the request path, so it can't add any
        delay to real users OR to outsiders — it's pure background cleanup.
        Authorized users' data is never touched.
        """
        while True:
            ...
            try:
                ...
                if removed:
                    ...
                    _release_chart_memory()  # hand the freed RAM back to the OS
