...
class UserTelegramSender:
    ...
    def start(self):
        async def init():
            self.client = TelegramClient('smz_user_session', USER_API_ID, USER_API_HASH)
            await self.client.start(phone=USER_PHONE)
            self.ready = True
            print(f"{Fore.GREEN}[✓] User Telegram (Premium) ready – using master account.{Style.RESET_ALL}")
            while True:
    ...
    def send_message(self, chat_id, text):
        ...
        try:
            ...
            print(f"{Fore.RED}❌ User send failed: {e}{Style.RESET_ALL}")
            return False
    ...
    def send_file(self, chat_id, file_path, caption):
        ...
        future = asyncio.run_coroutine_threadsafe(send_file(), self.loop)
        try:
            future.result(timeout=30)
            return True
        except Exception as e:
            print(f"{Fore.RED}❌ User send file failed: {e}{Style.RESET_ALL}")
            return False
        ...
        async def send():
            ...
            for ch in text:
                ...
                if eid:
                    entities.append(TelethonCustomEmoji(offset=offset, length=clen, document_id=eid))
                offset += clen
            # Add bold entity for the entire message
            entities.append(TelethonBold(offset=0, length=len(text)))
            await self.client.send_message(chat_id, text, formatting_entities=entities)
        future = asyncio.run_coroutine_threadsafe(send(), self.loop)
        try:
            future.result(timeout=15)
            ...
            print(f"{Fore.RED}❌ User send bold failed: {e}{Style.RESET_ALL}")
            return False
...
def run_checker_local(uid, date_str, signals_text):
    ...
    for sig in signals:
        ...
        if not entry:
            for hhmm in sorted(candle_dict.keys()):
                if hhmm >= time_str:
                    entry = candle_dict[hhmm]
                    break
        if not entry:
            continue
        ...
        if win1:
            results.append(("WIN", f"M1 {display} {time_str} {direction}"))
            continue
        ...
        if not next_candle:
            results.append(("LOSS", f"M1 {display} {time_str} {direction}"))
            continue
        ...
        if win2:
            results.append(("MTG_WIN", f"M1 {display} {time_str} {direction}"))
        else:
            results.append(("LOSS", f"M1 {display} {time_str} {direction}"))
    ...
    for status, line_text in results:
        if status == "WIN":
            signal_lines.append(f"{line_text} ✅")
            win_count += 1
        elif status == "MTG_WIN":
            signal_lines.append(f"{line_text} ✅¹")
            win_count += 1
    ...
    date_display = date_str_norm.replace('-', '.')
    # Header, separators, summary – fancy_font (bold‑like), signal lines normal
    header = (
        f"{fancy_font('▰▱▱ SMZ BOT CHECKER ▱▱▰')}\n"
        f"{fancy_font('              ┏━━━━━━━━━━━┓')}\n"
        f"{fancy_font('                 🗓 - ')}{fancy_font(date_display)}{fancy_font('          ')}\n"
        f"{fancy_font('              ┗━━━━━━━━━━━┛')}\n"
        f"{fancy_font('━━━━━━━━━━━ • ━━━━━━━━━━━')}\n"
    )
    body = "\n".join(signal_lines)
...
async def global_text_handler(
    ...
    if state == STATE_CHECKER_CUSTOM_DATE:
        ...
        run_checker_local(uid, date_str, text)   # remove context
        context.user_data['state'] = None
    ...
    while st.running:
        ...
        for pair in pairs:
            ...
            if bias:
                ...
                # ========== SIGNAL SENDING (format check) ==========
                if context.user_data.get('auto_signal_format') == 2:
                    # Format 2: text only, predefined style
                ...
                if context.user_data.get('auto_signal_format') == 2:
                    ...
                    # Format 1: original result with chart
                    if win1:
                        st.session_stats['wins'] += 1
                ...
                # Pause 15 seconds between signals
                for _ in range(15):
                    if st.stop_requested or st.paused:
                        break
                    t.sleep(1)
