...
def _run_sio_checker_thread(uid: int, context, signals_text: str):
    ...
    if not parsed:
        sender.send_message(uid, "❌ No valid signals found. Use format: M1;PAIR-OTC;HH:MM;CALL")
        return
    ...
    total_signals = len(parsed)
    # Send a single loading message
    loading_msg = sender.send_message(uid, f"⏳ 𝙶𝚎𝚗𝚎𝚛𝚊𝚝𝚒𝚗𝚐 𝚛𝚎𝚜𝚞𝚕𝚝𝚜 {'.'*0}")
    if not loading_msg:
        return
    msg_id = loading_msg.id

    # Function to update loading dots (run in background)
    stop_animation = threading.Event()
    def animate():
        dots = 0
        while not stop_animation.is_set():
            dots = (dots % 3) + 1
            text = f"⏳ 𝙶𝚎𝚗𝚎𝚛𝚊𝚝𝚒𝚗𝚐 𝚛𝚎𝚜𝚞𝚕𝚝𝚜 {'.'*dots}"
            sender.edit_message(uid, msg_id, text)
            ttime.sleep(1)
    anim_thread = threading.Thread(target=animate, daemon=True)
    anim_thread.start()

    all_results = []  # list of (api_line, result_icon)
    ...
    for api_date, sig_list in groups.items():
        ...
        if not check_id:
            error_occurred = True
            break
        result = _sio_checker_poll(check_id)
        if not result:
            error_occurred = True
            break
        ...
        import re
        signal_pattern = re.compile(r'^(M1|M2|M4|M5|M15|M30);[^;]+;\d{2}:\d{2};[A-Z]+')
        for s in raw_signals:
            if not signal_pattern.match(s):
                continue
            core = s
            if ";WIN" in s:
                icon = "✅"
                ...
                icon = "⏳"
            all_results.append((core, icon))

    # Stop animation and delete loading message
    stop_animation.set()
    ...
    if error_occurred:
        sender.send_message(uid, "❌ SMZ server is busy. Please try again later.")
        return

    result_map = {core: icon for core, icon in all_results}
    wins = sum(1 for icon in result_map.values() if icon == "✅")
...
def _sio_backtest_request(signals: list, start_date: str, end_date: str, gale: int, mode: str):
    if not signals:
        return None
    payload = {
        "info": {
    ...
    try:
        resp = requests.post(f"{SIO_API_BASE}/quotex/backtest", json=payload, headers=headers, timeout=30)
        if resp.status_code != 200:
            return None
        data = resp.json()
        if not data.get("status"):
...
def _sio_backtest_poll(bt_id: str, max_retries=60, delay=3):
    headers = {
        "X-API-Key": SIO_API_KEY,
        "Referer": "https://sio.tools/"
    }
    for _ in range(max_retries):
        try:
            resp = requests.get(f"{SIO_API_BASE}/quotex/backtest/{bt_id}", headers=headers, timeout=15)
            if resp.status_code != 200:
                time.sleep(delay)
                continue
            data = resp.json()
...
def _run_sio_backtest_thread(uid: int, context, signals_text: str):
    ...
    # Use exactly the selected days (not 30)
    start_date_user = today_user - timedelta(days=days)
    end_date_user = today_user - timedelta(days=1)   # yesterday inclusive
    print(f"DEBUG: user range {start_date_user} to {end_date_user}")

    # Convert to UTC-3 (SIO timezone)
    tz_sio = timezone(timedelta(hours=SIO_ORIGINAL_TZ))
    start_midnight = datetime.combine(start_date_user, datetime.min.time(), tzinfo=tz_user)
    end_midnight = datetime.combine(end_date_user, datetime.min.time(), tzinfo=tz_user)
    start_sio = start_midnight.astimezone(tz_sio).date()
    end_sio = end_midnight.astimezone(tz_sio).date()
    start_date_str = start_sio.strftime("%Y-%m-%d")
    end_date_str = end_sio.strftime("%Y-%m-%d")
    print(f"DEBUG: SIO range {start_date_str} to {end_date_str}")

    # Parse signals
    sio_signals = _bt2_parse_signals(signals_text)
    print(f"DEBUG: parsed {len(sio_signals)} signals")
    if not sio_signals:
        sender.send_message(uid, "❌ No valid signals found.")
        return

    # Loading animation
    loading_msg = sender.send_message(uid, "⏳ Processing backtest...")
    if not loading_msg:
        return
    msg_id = loading_msg.id
    ...
    def animate():
        ...
        while not stop_anim.is_set():
            dots = (dots % 3) + 1
            text = f"⏳ Processing backtest{'.' * dots}"
            sender.edit_message(uid, msg_id, text)
            ttime.sleep(1)
    anim_thread = threading.Thread(target=animate, daemon=True)
    anim_thread.start()

    bt_id = _sio_backtest_request(sio_signals, start_date_str, end_date_str, gale, "geral")
    print(f"DEBUG: bt_id = {bt_id}")
    if not bt_id:
        stop_anim.set()
        anim_thread.join(timeout=1)
    ...
    # Poll longer (60 tries * 3s = 180s)
    result = _sio_backtest_poll(bt_id, max_retries=60, delay=3)
    stop_anim.set()
    anim_thread.join(timeout=1)

    if not result:
        sender.edit_message(uid, msg_id, "❌ Backtest timeout. Please try again with fewer signals or a smaller date range.")
        return

    raw_signals = result.get("signals", [])
    print(f"DEBUG: raw signals count = {len(raw_signals)}")
    wins, losses = [], []
    for s in raw_signals:
        parts = s.split(";")
        if len(parts) >= 5:
            ...
            if "WIN" in status:
                wins.append(f"{tf} {pair} {tm} {direc}")
            elif "LOSS" in status:
                losses.append(f"{tf} {pair} {tm} {direc}")

    sender.edit_message(uid, msg_id, "✅ Backtest complete!")
...
async def _show_bt2_mtg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        [colored_button(" Gale 0 (none)", "bt2_mtg_0", KeyboardButtonStyle.PRIMARY, "6147767443910564861"),
         colored_button(" Gale 1", "bt2_mtg_1", KeyboardButtonStyle.SUCCESS, "6336932114457435936")],
        [colored_button(" Gale 2", "bt2_mtg_2", KeyboardButtonStyle.PRIMARY, "6150120471808449073"),
...
async def global_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== Backtest 2.0 States =====
        ...
        await update.message.reply_text("⏳ Backtest started... (results will appear in a moment)")
        return
