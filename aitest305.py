        ...
        async def init():
            try:
                ...
                if not session_string:
                    ...
                    # copied into Render's env var (TELETHON_BOT_SESSION).
                    # After that, every future boot reuses it -> no more
                    # fresh bot-auth calls -> no more FloodWaitError.
                    try:
                        exported = StringSession.save(self.client.session)
                        print(f"{Fore.YELLOW}{'=' * 80}\n"
                              f"[ACTION NEEDED] Copy this into Render env var TELETHON_BOT_SESSION:\n"
                              f"{exported}\n"
