...
async def is_authorized_async(uid: int) -> bool:
    ...
    the network."""
    return await asyncio.to_thread(is_authorized, uid)
...
async def run_blackout_fs(uid, start_time, end_time, pairs_list):
    ...
    for pair in pairs_list:
        ...
        times = await asyncio.to_thread(fetch_blackout_signals, pair, start_time, end_time)
        if times:
            for t in times:
...
async def chart_analyzer_photo_handler(
    ...
    try:
        ...
        result = await asyncio.to_thread(_gemini_analyze_chart, bytes(photo_bytes))
