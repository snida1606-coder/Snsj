...
# ─── Dedicated thread pools to stop fast checks getting stuck behind
# slow network calls ──────────────────────────────────────────────
# This container has very few CPUs, so Python's DEFAULT background-thread
# pool (the one `asyncio.to_thread()` uses everywhere by default) is sized
# as min(32, cpu_count + 4) — as small as 5 threads. Every asyncio.to_thread
# call in the whole bot (is_authorized checks, backtest/blackout fetches,
# per-pair payout fetches when loading Multi Engine's pair list — 20-35 of
# these fired at once via asyncio.gather, and the 10-60s+ chart analyzer)
# was sharing that same tiny pool. Any burst of slow calls (e.g. a Multi
# Engine pair-list load) fully occupied all 5 threads for many seconds, so
# something as cheap as an is_authorized() cache-miss check for a
# completely different user had to sit in line behind them — this is what
# was causing multi-minute stalls on things like /start or "Enter start
# time" even after making the checks themselves non-blocking.
# _AUTH_EXECUTOR is small but DEDICATED — is_authorized_async never has to
# wait behind anything slow, no matter how busy the rest of the bot is.
_AUTH_EXECUTOR = _cf.ThreadPoolExecutor(max_workers=8, thread_name_prefix="auth-check")
# _NETWORK_EXECUTOR is a generously sized dedicated pool for every other
# asyncio.to_thread() call (candle/payout fetches, backtest, blackout,
# login, chart analyzer) so they no longer compete with each other OR with
# auth checks for the same handful of slots.
_NETWORK_EXECUTOR = _cf.ThreadPoolExecutor(max_workers=48, thread_name_prefix="net-io")
...
# Multiple workers instead of one: with a single dispatcher thread, ANY one
# slow/retrying send (a Telegram TimedOut can legitimately eat ~30s across
# its retries) blocked EVERY other user's messages behind it in the same
# queue — including /start, /stop, everything — since the next queued item
# was never even picked up until the current one fully finished. A handful
# of workers pulling from the same queue means one stuck chat's send no
# longer stalls every other chat; the shared pacing lock still keeps
# per-chat/global rate limits correct across all of them.
# Sized for ~25 concurrent active users (Auto Signal/Multi Engine running
# at once): even if a handful of them hit a slow/retrying send at the same
# moment, there are still enough free workers left that the rest of the
# users don't notice any delay.
_SEND_DISPATCHER_WORKERS = 20
for _i in range(_SEND_DISPATCHER_WORKERS):
    threading.Thread(target=_send_dispatcher_loop, daemon=True,
                      name=f"send-dispatcher-{_i}").start()
...
class PTBSender:
    ...
    def _run_with_flood_retry(self, coro_factory, timeout=30, max_retries=6):
        ...
        # Guard against self-deadlock: if this is being called from a
        # coroutine that is already executing ON MAIN_LOOP's own thread
        # (e.g. an async command/callback handler that calls
        # sender.send_message(...) directly instead of using the PTB
        # `context.bot` / `await` API), then run_coroutine_threadsafe()
        # schedules work onto a loop that can't run it — that thread is
        # the one blocked waiting on future.result(). That wait always
        # times out (~30s) and surfaces as a bare "Send error: " with an
        # empty message (a bare concurrent.futures.TimeoutError), which is
        # exactly what was making things like /stop feel stuck. When we
        # detect that situation, schedule the send as a fire-and-forget
        # task on the loop instead of blocking for its result.
        try:
            running_loop = asyncio.get_running_loop()
        except RuntimeError:
            running_loop = None
        if running_loop is not None and running_loop is MAIN_LOOP:
            print("[SEND] called from MAIN_LOOP's own thread — "
                  "dispatching without blocking to avoid self-deadlock")
            MAIN_LOOP.create_task(coro_factory())
            return None

        future = None
        for attempt in range(max_retries):
            # Only schedule a NEW send if we don't already have one in
            # flight. Previously, every branch of this loop (including the
            # local-timeout case below) called coro_factory() again and
            # scheduled a brand-new future WITHOUT cancelling/forgetting
            # the old one. The old future's coroutine keeps running on
            # MAIN_LOOP regardless of whether this thread gave up waiting
            # on it — so once MAIN_LOOP got busy (any backlog, not just a
            # full freeze), the ORIGINAL send would still go through on
            # its own AND every retry scheduled its own duplicate send.
            # Net result: the same message (e.g. "Enter start time")
            # landing 2-6 times at once whenever MAIN_LOOP caught up after
            # a slow patch. Now we keep waiting on the SAME future for
            # local timeouts, and only create a fresh one when we've
            # confirmed (via an actual exception) that the previous
            # attempt is really finished.
            if future is None:
            ...
            try:
                ...
                # BadRequest is a subclass of NetworkError in PTB, so without
                # this it falls into the retry branch below — but things
                # like "message to edit not found", "message is not
                # modified", or "query is too old" are PERMANENT: the
                # message/query is gone and retrying (6x, ~30s+ per call)
                # can never succeed. That was making progress-bar edit
                # loops hammer the API for tens of seconds on every single
                # update and stall the whole bot. Fail fast instead.
                print(f"[SEND SKIP] BadRequest (non-retryable): {e}")
                return None
                ...
                # future.result(timeout=...) itself timing out — this means
                # MAIN_LOOP took longer than `timeout`s to even get around
                # to running the coroutine (it was busy with something
                # else), NOT that Telegram rejected anything. This is a
                # bare concurrent.futures.TimeoutError with an empty
                # message, which is why this used to print as a blank
                # "Send error:" and give up immediately with no retry —
                # worst possible combination (silent AND permanent). Treat
                # it like a transient network error instead: keep waiting
                # on the SAME future (do NOT create a duplicate send).
                print(f"[SEND RETRY] still waiting for MAIN_LOOP "
                      f"(attempt {attempt+1}/{max_retries})")
                time.sleep(1.5 * (attempt + 1))
                continue
            except Exception as e:
                # Always log the exception TYPE, not just str(e) — some
                # exceptions (like the TimeoutError above) stringify to
                # nothing, and a bare "Send error:" with no other info is
                # nearly impossible to debug from Railway logs alone.
...
async def run_backtest_local(
    ...
    for sig in signals:
        ...
        try:
            ...
            resp = await MAIN_LOOP.run_in_executor(_NETWORK_EXECUTOR, lambda: requests.get(url, timeout=20))
            if resp.status_code != 200:
...
def fetch_candle_at_epoch_with_retry(pair, target_epoch, count=50, timeframe="1m",
                                      tolerance=45, attempts=25, delay=3):
...
def main():
    ...
    app = Application.builder().token(BOT_TOKEN) \
        ...
        .build()
    # concurrent_updates(64) — THE single biggest fix for "bot slow with
    # many users". By default PTB's Application processes updates
    # SEQUENTIALLY: one user's entire handler (including every `await`
    # inside it — get_file, send_photo, DB calls, anything) must fully
    # finish before the Application even starts looking at the NEXT
    # user's message, regardless of how many background threads exist for
    # sending. With this bot's ~25+ concurrent Auto Signal / Multi Engine
    # users, that meant every user's /start, button tap, or text reply sat
    # behind however many other users happened to be mid-handler at that
    # moment. concurrent_updates(64) lets up to 64 handlers run truly in
    # parallel on MAIN_LOOP (they're async, so this is genuine concurrency,
    # not extra threads/processes) — one user's slow step no longer
