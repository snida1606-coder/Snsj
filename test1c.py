...
class SMZXBot:
    ...
    def fetch_candle_at_time_with_retry(
            self, pair, target_dt_utc5, limit=750, attempts=5, delay=2, timeframe="M1"):
        """
...
def fetch_candle_at_epoch_with_retry(pair, target_epoch, count=50, timeframe="1m",
                                      tolerance=45, attempts=15, delay=2):
            ...
            for pair in pairs:
                ...
                if bias:
                    ...
                    candles2, first = bot.fetch_candle_at_time_with_retry(
                        pair, entry_dt, limit=50, delay=1, timeframe=tf)
                    if not candles2:
                        sender.send_message(
                            st.target_chat,
                    ...
                    if win1:
                        ...
                        candles3, second = bot.fetch_candle_at_time_with_retry(
                            pair, entry_dt + timedelta(minutes=tf_minutes), limit=50, delay=1, timeframe=tf)
                        if not candles3:
                            sender.send_message(
                                st.target_chat,
