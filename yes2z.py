...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # The callback query can expire (~15-30s) before we get to it if the
        # bot was briefly congested — that used to abort this ENTIRE handler
        # right here, so the button click just did nothing (no menu, no
        # error visible to the user). answer() failing doesn't mean the
        # button press itself is invalid, so log it and keep going instead
        # of dropping the whole interaction.
